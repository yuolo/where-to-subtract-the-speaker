...
rank truncation. The code and the CSVs in `outputs/` preserve those historical
experiments unchanged. Both comparisons were then rerun under the corrected
paths, and those runs are in `audit_reruns/`. The CRNN placement, depth,
and fixed-enrollment experiments do not use these two paths.

## Audit reruns

```bash
python audit_rerun_analysis.py   # both controls against the released runs
```

`audit_reruns/` holds the two controls for the issues listed above. They use
the folds and seeds of the released runs, so every comparison is paired.

The fine-tuned arms were rerun with the same input masking on both sides. The
operator keeps its lower-leakage sign on all three corpora. On CREMA-D the gap
is -.038 bits against -.043 before, but every run now carries it (p = .0039
against p = .02) and the operator moves ahead on UAR as well. On RAVDESS the
gap widens from -.011 to -.081 bits and turns significant (p = .035 against
p = .69), again with the operator ahead on UAR. On IEMOCAP the gap keeps its
size, -.017 against -.018 bits, but it no longer reaches significance
(p = .30). The masking asymmetry had been working in favor of input-CMN.

INLP was rerun with each projection restricted to the numerical rank of the
speaker classifier. This rerun retrains the plain encoder as well, so the four
other post-hoc variants bound the run-to-run spread. On CREMA-D leakage rises
from -0.107 to -0.018 bits while UAR falls from 0.233 to 0.215. RAVDESS moves
from -0.329 to -0.527 bits with UAR within .001. IEMOCAP holds its UAR at
0.553 and moves .006 bits, which is what a binary speaker probe predicts, as
the untruncated factorization already had the right rank there. The reading in
Sec. 3.4 stands, since INLP still reaches the control level only by giving up
most of the task.

The fine-tuned reruns ran on two machines with the same pinned library
versions, CREMA-D on one and RAVDESS and IEMOCAP on the other. Both arms of a
corpus come from the same machine.
